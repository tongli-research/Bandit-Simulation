...
from . import bayes_vector_ops as bayes
...
class SimulationConfig:
    ...
    arm_reward_custom_dist (list of lists, optional): User-specified discrete reward distributions for each arm.
    If provided, this overrides `reward_model`, `arm_mean_reward_dist_loc`, and `arm_mean_reward_dist_scale`.
    Each element `arm_reward_custom_dist[i]` is a list of possible reward values for arm `i`,
    and every time arm `i` is pulled, the reward is sampled uniformly from this list.
    Example:
        arm_reward_custom_dist = [
            [0, 1],              # Arm 0 samples from {0,1}
            [0.2, 0.5, 1.0],     # Arm 1 samples from {0.2, 0.5, 1.0}
        ]
    ...
    arm_reward_custom_dist: Optional[list] = None  # [GUI_INPUT]
    arm_mean_reward_dist_spec: dict = field(default_factory=lambda: {
        "dist": "normal",  # name of np.random.<dist>
        "params": {
            "loc": 0.5,
            "scale": 0.15,
        }
    ...
    determined_reward_trajectory = None  # (reward, reward2), shapes: (n_rep, T, K)
    ...
    def manual_init(self): #TODO: can we remove manual init? Maybe not..
        ...
        if "dist" not in spec or "params" not in spec:
            raise ValueError("arm_mean_reward_dist must have 'dist' and 'params'")

        if not hasattr(np.random, spec["dist"]):
            raise ValueError(f"np.random has no distribution '{spec['dist']}'")
        ...
        if self.arm_reward_custom_dist is not None:
            warnings.warn(
                "Using arm_reward_custom_dist: ignoring mean/var reward distribution settings."
            )

    def _get_step_schedule(self):
        t=self.burn_in_per_arm * self.n_arm
        if self.burn_in_per_arm>0:
            step_schedule_list = [self.burn_in_per_arm* self.n_arm]
            sample_batch_schedule_list = [self.burn_in_per_arm]
        else:
            step_schedule_list = []
            sample_batch_schedule_list = []
        while t<self.horizon:
            sample_batch_size = math.floor(1 + t**(1/3)*self.batch_scaling_rate*5 )
            step_size = math.floor(self.base_batch_size + t*self.batch_scaling_rate)
    ...
    def _broadcast_param(self, value, name):
        if isinstance(value, list):
            if len(value) != self.n_arm:
                raise ValueError(f"{name} must have length n_arm")
            return value
        return [value] * self.n_arm
    ...
    @property
    def arm_mean_reward_dist(self):
        np.random.seed(0) #must have, to make sure arm_mean_reward_dist is not random each time
        spec = self.arm_mean_reward_dist_spec
        dist_name = spec["dist"]  # e.g. "normal", "beta"
        params = spec.get("params", {})

        # Resolve numpy RNG function
        if not hasattr(np.random, dist_name):
            raise ValueError(f"np.random has no distribution '{dist_name}'")

        rng_fn = getattr(np.random, dist_name)

        # Broadcast params (scalar → per-arm list)
        params = {
            k: self._broadcast_param(v, k)
    ...
    def generate_full_reward_trajectory(self, arm_mean_reward_dist = None):
        ...
        if self.arm_reward_custom_dist is not None:
            ...
            reward = np.zeros((T, n_rep, n_arm))
            reward2 = np.zeros((T, n_rep, n_arm))

            for arm in range(n_arm):
                values = np.array(self.arm_reward_custom_dist[arm])
                samples = np.random.choice(values, size=(T, n_rep))
                reward[:, :, arm] = samples
                reward2[:, :, arm] = samples ** 2

            if self.compact_array:
                reward = self._get_compact_array(reward)
                reward2 = self._get_compact_array(reward2)

            reward = np.moveaxis(reward, 0, 1)
            reward2 = np.moveaxis(reward2, 0, 1)
        ...
        if self.reward_model.__name__ == 'binomial':
            ...
            params = {'loc': arm_mean_reward_dist, 'scale': self.reward_std, 'size':(self.horizon,self.n_rep,self.n_arm)}
        else:
