...
class TControl(TestProcedure):
    ...
    def compute_power(self,
        ...
        # test_result_naive = np.abs(h1_test_stat) > 1.96
        # test_result_naive = test_result_naive * 1.0
        # test_result_naive [~expanded_mask] = np.nan
        # power_naive  = np.nanmean(test_result_naive , axis=(0, 2))
...
class Tukey(TestProcedure):
    ...
    def _filter_stats(self,stat_reordered,method='max'):
        ...
        if method == 'max':
            flattened = np.nanmax(stat_reordered[h_idx, r_idx, i_idx, :], axis=-1)
        elif method == 'min':
    ...
    def get_critical_region(self, test_stat):
        ...
        #     crit_boundary = np.nanquantile(flattened, q=1 - self.type1_error_constraint, axis=1, keepdims=True) #TODO: see why it gets smaller as T increase
    ...
    def compute_power(self,
        ...
        if self.test_type == 'distinct-best-arm':
            ...
            sorted_h1_test_stat = self._filter_stats(stat_reordered,method='min')
